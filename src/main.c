  ...
  if (event->source_type == INPUT_SOURCE_KEYBOARD) {
    if (event->key_type == INPUT_KEY_CHAR) {
      ...
      if (event->key_code == KEY_BACKSPACE) {
        ...
        lr_file_split(&phantom.buffer->lr, phantom.cursor.line_no,
                      phantom.cursor.pos);
        ...
        phantom.cursor.owner = lr_owner_find(&phantom.buffer->lr,
                                             lr_owner(phantom.cursor.line_no));
...
void resource_load() {
  ...
  font_default =
      font_load("assets/fonts/FiraCode-Regular.ttf", "assets/shaders/sdf.fs");
